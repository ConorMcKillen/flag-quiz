...
let countries = JSON.parse(JSON.stringify(originalCountries));
...
function classToggle() {
  ...
  document.querySelector(".game-section").style.gap = "0px";
}
...
function handleHomeScreenButtonClick() {
  countries = JSON.parse(JSON.stringify(originalCountries));
...
function handleNextButtonClick() {
  ...
  if (guessedCountryName === correctCountryName) {
    ...
    if (
      (oceaniaSelected === true &&
        guessedCountries.length < originalCountries.Oceania.length) ||
      (europeSelected === true &&
        guessedCountries.length < originalCountries.Europe.length) ||
      (africaSelected === true &&
        guessedCountries.length < originalCountries.Africa.length) ||
      (asiaSelected === true &&
        guessedCountries.length < originalCountries.Asia.length) ||
      (americasSelected === true &&
        guessedCountries.length < originalCountries.Americas.length)
    ) {
      fetchFlagByCountryName(countryName)
        .then((flagUrl) => {
          if (flagUrl) {
...
function handleSkipButtonClick() {
  ...
  if (
    ...
    (africaSelected &&
      guessedCountries.length < originalCountries.Africa.length) ||
    (asiaSelected && guessedCountries.length < originalCountries.Asia.length) ||
    (americasSelected &&
      guessedCountries.length < originalCountries.Americas.length)
  ) {
    fetchFlagByCountryName(countryName)
